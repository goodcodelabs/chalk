...
async function load() {
  ...
  try {
    const [threads, history] = await Promise.all([
      api.listAgentThreads(props.agentId),
      api.agentThreadHistory(props.threadId),
    ])
    const t = threads.find(t => t.id === props.threadId)
    if (t) threadName.value = t.name
...
async function send() {
  ...
  try {
    const { job_id } = await api.agentChat(props.threadId, msg)
    while (true) {
